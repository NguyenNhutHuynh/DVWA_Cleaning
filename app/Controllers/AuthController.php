...
final class AuthController
{
     ...
     */
    /**
     * Hiển thị biểu mẫu quên mật khẩu.
     */
     ...
     * Luôn trả về thông báo chung (không tiết lộ user có tồn tại hay không).
     */
    public static function sendResetLink(): void
    {
        ...
        // Luôn trả về tin nhắn chung (không tiết lộ user có tồn tại)
        if (!$user) {
            self::setSessionMessage('success', 
        ...
        // Tạo token reset
        PasswordReset::deleteOldTokens((int)$user['id']);
        $token = PasswordReset::create((int)$user['id']);
        
        // Gửi email
        $resetLink = getenv('APP_BASE_URL') . '/reset-password/' . $token;
        self::sendPasswordResetEmail($user['email'], $user['name'], $resetLink);
    ...
    /**
     * Hiển thị biểu mẫu đặt lại mật khẩu (sau khi người dùng nhấn link từ email).
     *
     * @param string $token Token reset từ URL
     */
    public static function showResetPassword(string $token): void
    {
        $user = PasswordReset::findUserByToken($token);
    ...
    public static function resetPassword(): void
    {
        ...
        self::setSessionMessage('success', 'Đặt lại mật khẩu thành công. Vui lòng đăng nhập.');
        self::redirect('/login');
    }
     ...
     * @param string $resetLink Link reset đầy đủ
     */
    private static function sendPasswordResetEmail(string $email, string $name, string $resetLink): void
    {
        $subject = 'Đặt lại mật khẩu';
        $body = "Xin chào {$name},\n\n"
            . "Bạn yêu cầu đặt lại mật khẩu. Nhấn link dưới đây:\n\n"
            . "{$resetLink}\n\n"
            . "Link sẽ hết hạn trong 1 giờ.\n\n"
            . "Nếu không phải bạn yêu cầu, vui lòng bỏ qua email này.";
        
        mail($email, $subject, $body, 'From: noreply@cleaning.test');
    }
